...
class SyncChecker {
  ...
  private checkGitStatus(): void {
    try {
      ...
      if (status.trim()) {
        ...
        const behind = execSync('git rev-list HEAD..origin/main --count', { cwd: this.projectRoot }).toString().trim()
        const ahead = execSync('git rev-list origin/main..HEAD --count', { cwd: this.projectRoot }).toString().trim()
  ...
  private checkDataFiles(): void {
    ...
    try {
      ...
      const lastModified = execSync(`git log -1 --format=%cd --date=relative ${dataPath}`, { cwd: this.projectRoot }).toString().trim()
      
      // ツール数をカウント
      const toolsMatch = content.match(/export const tools = \[([\s\S]*?)\]/);
      const toolCount = toolsMatch ? (toolsMatch[1].match(/id:/g) || []).length : 0
  ...
  private generateReport(): void {
    ...
    const outOfSync = this.syncResults.filter(r => r.status === 'out_of_sync')
    ...
    if (outOfSync.length > 0) {
      ...
      if (this.syncResults.find(r => r.component === 'Git Repository' && r.status === 'out_of_sync')) {
        console.log('  1. git add . && git commit -m "同期用コミット"')
        console.log('  2. git pull origin main')
        console.log('  3. git push origin main')
      }
      
      if (this.syncResults.find(r => r.component === 'Agent Config' && r.status === 'out_of_sync')) {
  ...
  public async run(): Promise<void> {
    try {
      ...
      // 終了コード
      const hasOutOfSync = this.syncResults.some(r => r.status === 'out_of_sync')
