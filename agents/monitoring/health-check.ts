...
class AgentHealthChecker {
	...
	public checkResearchAgent(): HealthCheckResult {
		...
		checks.push({
			...
			message: reportsExists
				? "✅ reportsディレクトリ存在"
				: "⚠️ reportsディレクトリなし",
		});
		if (!reportsExists && overallStatus === "healthy")
		...
		if (reportsExists) {
			...
			if (jsonFiles.length > 0) {
				...
				if (ageHours > 24 && overallStatus === "healthy")
					overallStatus = "warning";
			} else {
				checks.push({
					name: "最新レポート",
		...
		if (existsSync(logPath)) {
			try {
				...
				if (lastUpdate) {
					...
					if (ageDays > 7 && overallStatus === "healthy")
						overallStatus = "warning";
				}
			} catch (error) {
				checks.push({
